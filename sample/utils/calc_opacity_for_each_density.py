...
def load_2d_opacity_table(filename):
    ...
    with h5py.File(filename, 'r') as f:
        ...
        log_rho_len = len(data['log_density'])
        print(f"  Log temperature grid: {data['log_temperature'].shape} - "
              f"{log_temp_len} points")
        print(f"  Log density grid: {data['log_density'].shape} - "
              f"{log_rho_len} points")
...
def interpolate_2d_opacity(data, target_temp, target_rho):
    """
    Interpolate opacity at given temperature and density using 2D grid
    ...
    if log_target_rho <= log_rho_grid[0]:
        j1, j2 = 0, 0
        r_frac = 0.0
    elif log_target_rho >= log_rho_grid[-1]:
        j1, j2 = len(log_rho_grid)-1, len(log_rho_grid)-1
        r_frac = 0.0
    else:
        j2 = np.searchsorted(log_rho_grid, log_target_rho)
        j1 = j2 - 1
        rho_diff = log_rho_grid[j2] - log_rho_grid[j1]
        r_frac = (log_target_rho - log_rho_grid[j1]) / rho_diff
        print("j1, j2 = ", j1, j2)
        print("r_frac = ", r_frac)

    # Bilinear interpolation on 2D grid: opacity[log_density_idx, log_temp_idx]
    planck_grid = data['planck_mean_opacity']
    ross_grid = data['rosseland_mean_opacity']

    # Get corner values - NOTE: grid is [log_density, log_temperature]
    ...
    x2norm = (len(log_rho_grid) - 1) / (log_rho_grid[-1] - log_rho_grid[0])
    
    t = (log_target_temp - log_temp_grid[0]) * x1norm
    r = (log_target_rho - log_rho_grid[0]) * x2norm
...
def demonstrate_2d_table(data_folder, output_folder):
    ...
    rho_min = 10**data['log_density'].min()
    rho_max = 10**data['log_density'].max()

    print(f"Temperature range: {temp_min:.2e} - {temp_max:.2e} K")
    print(f"Density range: {rho_min:.2e} - {rho_max:.2e} g/cm³")
    ...
    # test_points = [
    #     (1e4, 1e-12),   # Low temperature, low density
    #     (1e6, 1e-6),    # High temperature, high density
    #     (5623, 7e-11),  # Mid-range values
    # ]

    # for temp, rho in test_points:
    #     planck, ross = interpolate_2d_opacity(data, temp, rho)
    #     print(f"T = {temp:.0e} K, ρ = {rho:.0e} g/cm³:")
    ...
    # rho_linear = 10**data['log_density']

    # # Temperature vs Density grid points
    # T_grid, R_grid = np.meshgrid(temp_linear, rho_linear,
    #                              indexing='ij')

    # # 1. Temperature-Density grid
    # ax1.scatter(T_grid.flatten(), R_grid.flatten(), s=20, alpha=0.7)
    # ax1.set_xscale('log')
    # ax1.set_yscale('log')
    # ax1.set_xlabel('Temperature [K]', fontsize=label_fontsize)
    # ax1.set_ylabel('Density [g/cm³]', fontsize=label_fontsize)
    ...
    #                  extent=[data['log_density'].min(),
    #                          data['log_density'].max(),
    ...
    # # 4. Opacity comparison at different temperatures (mid density slice)
    # mid_density_idx = len(data['log_density']) // 2
    # mid_rho = 10**data["log_density"][mid_density_idx]
    # # planck_label = r'Planck ($\rho$={{:.0e}} $\mathrm{g}\,\mathrm{cm}^{-3}$)'.format(mid_rho)
    # # ross_label = r'Rosseland ($\rho$={:.0e} $\mathrm{g}\,\mathrm{cm}^{-3}$)'.format(mid_rho)
    # planck_label = f'Planck ($\\rho$={mid_rho:.0e} g/cm$^3$)'
    # ross_label = f'Rosseland ($\\rho$={mid_rho:.0e} g/cm$^3$)'
    # ax4.loglog(temp_linear,
    #            data['planck_mean_opacity'][mid_density_idx, :],
    #            'o-',
    #            label=planck_label,
    #            markersize=4)
    # ax4.loglog(temp_linear,
    #            data['rosseland_mean_opacity'][mid_density_idx, :],
    #            's-',
    #            label=ross_label,
    #            markersize=4)
    # # ax4.set_xlabel('Temperature [K]')
    # # ax4.set_ylabel('Opacity [cm²/g]')
    # # ax4.set_title('Opacity vs Temperature (Mid Density)')
    # ax4.set_xlabel('Temperature [K]', fontsize=label_fontsize)
    # ax4.set_ylabel('Opacity [cm²/g]', fontsize=label_fontsize)
    # # ax4.set_title('Opacity vs Temperature (Mid Density)', fontsize=title_fontsize)
    ...
    tmp_density = 1e-10
    tmp_tempature = 1e5
    planck_interp, ross_interp = interpolate_2d_opacity(data, tmp_tempature, tmp_density)
    print(f"\nInterpolated Planck opacity at T={tmp_tempature:.0e} K, "
          f"rho={tmp_density:.0e} g/cm³: {planck_interp:.6e} cm²/g")
    print(f"Interpolated Rosseland opacity at T={tmp_tempature:.0e} K, "
          f"rho={tmp_density:.0e} g/cm³: {ross_interp:.6e} cm²/g")
