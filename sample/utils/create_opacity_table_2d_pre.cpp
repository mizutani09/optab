...
// Creates efficient 2D grid structure: unique temperature array, unique pressure array,
// and 2D opacity arrays indexed by [temp_index][pressure_index]
...
const double K_BOL = 1.3806503e-16;
...
bool extract_data_from_file(const char* filepath, OpacityData& data) {
    ...
    // Extract pressure
    if (success && H5Lexists(file_id, "/nden", H5P_DEFAULT) > 0) {
        hid_t dataset_id = H5Dopen(file_id, "/nden", H5P_DEFAULT);
        hid_t dataspace_id = H5Dget_space(dataset_id);
        hssize_t num_elements = H5Sget_simple_extent_npoints(dataspace_id);
        std::vector<double> nden_data(num_elements);
        if (dataset_id >= 0) {
            if (H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, nden_data.data()) < 0) {
                success = false;
            }
            H5Dclose(dataset_id);
        } else {
        ...
        if (nden_data.size() > 0) {
            data.nden = nden_data[0];  // Just take the first element for now
            data.pressure = data.nden * K_BOL * data.temperature; // Convert nden to pressure
        }
    } else {
        success = false;
    }
...
bool create_2d_opacity_table(const std::vector<OpacityData>& data, const char* output_filename) {
    ...
    // Check if all temperature groups have the same number of pressure points
    size_t expected_pressure_count = temp_groups.begin()->second.size();
    bool uniform_structure = true;

    for (const auto& group : temp_groups) {
        if (group.second.size() != expected_pressure_count) {
    ...
    if (!uniform_structure) {
        ...
        hid_t pre_dataset = H5Dcreate2(file_id, "/pressure", H5T_NATIVE_DOUBLE, space_id,
                                       H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        if (pre_dataset >= 0) {
            H5Dwrite(pre_dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, pressures.data());
            H5Dclose(pre_dataset);
        }
    ...
    // Extract unique temperatures and pressures (assumes uniform structure)
    std::vector<double> temperatures;
    std::vector<double> pressures;
    std::vector<double> log_temperatures;
    std::vector<double> log_pressures;
    ...
    // Get pressures from the first temperature group (should be same for all)
    auto first_group = temp_groups.begin()->second;
    std::sort(first_group.begin(), first_group.end(),
              [](const OpacityData& a, const OpacityData& b) { return a.pressure < b.pressure; });

    for (const auto& d : first_group) {
        pressures.push_back(d.pressure);
        log_pressures.push_back(log10(d.pressure));
    }

    size_t n_temp = temperatures.size();
    size_t n_pre = pressures.size();

    printf("Grid: %zu temperatures x %zu pressures = %zu points\n", n_temp, n_pre, n_temp * n_pre);

    // Create 2D arrays for opacities (pressure first, then temperature)
    std::vector<std::vector<double>> planck_2d(n_pre, std::vector<double>(n_temp, 0.0));
    std::vector<std::vector<double>> rosseland_2d(n_pre, std::vector<double>(n_temp, 0.0));

    // Fill the 2D arrays [pressure_idx][temp_idx]
    for (size_t i = 0; i < n_temp; i++) {
        ...
        for (size_t j = 0; j < n_pre; j++) {
            if (j < temp_data.size()) {
                planck_2d[j][i] = temp_data[j].planck_mean;      // [pressure_idx][temp_idx]
                rosseland_2d[j][i] = temp_data[j].rosseland_mean; // [pressure_idx][temp_idx]
            }
        }
    }
    ...
    // Create 1D dataspaces for temperature and pressure arrays
    hsize_t temp_dims[1] = {n_temp};
    hsize_t pre_dims[1] = {n_pre};
    hid_t temp_space = H5Screate_simple(1, temp_dims, NULL);
    hid_t pre_space = H5Screate_simple(1, pre_dims, NULL);

    // Create 2D dataspace for opacity arrays (pressure × temperature)
    hsize_t opacity_dims[2] = {n_pre, n_temp};
    ...
    // Create and write pressure dataset (log10 values)
    hid_t pre_dataset = H5Dcreate2(file_id, "/log_pressure", H5T_NATIVE_DOUBLE, pre_space,
                                   H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (pre_dataset >= 0) {
        H5Dwrite(pre_dataset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, log_pressures.data());
        H5Dclose(pre_dataset);
    }

    // Flatten 2D arrays for HDF5 storage (row-major order: pressure × temperature)
    std::vector<double> planck_flat(n_pre * n_temp);
    std::vector<double> rosseland_flat(n_pre * n_temp);
    ...
    // Log pressure units and description
    if (H5Lexists(file_id, "/log_pressure", H5P_DEFAULT) > 0) {
        hid_t pre_id = H5Dopen(file_id, "/log_pressure", H5P_DEFAULT);
        hid_t attr = H5Acreate2(pre_id, "units", str_type, attr_space, H5P_DEFAULT, H5P_DEFAULT);
        const char* pre_units = "log10(erg/cm^3)";
        H5Awrite(attr, str_type, &pre_units);
        H5Aclose(attr);

        attr = H5Acreate2(pre_id, "description", str_type, attr_space, H5P_DEFAULT, H5P_DEFAULT);
        const char* pre_desc = "Log10 of pressure grid points";
        H5Awrite(attr, str_type, &pre_desc);
        H5Aclose(attr);
        H5Dclose(pre_id);
    }
    ...
    if (H5Lexists(file_id, "/planck_mean_opacity", H5P_DEFAULT) > 0) {
        ...
        const char* planck_desc = "Planck mean opacity [log_pressure, log_temperature]";
    ...
    if (H5Lexists(file_id, "/rosseland_mean_opacity", H5P_DEFAULT) > 0) {
        ...
        const char* ross_desc = "Rosseland mean opacity [log_pressure, log_temperature]";
    ...
    const char* grid_desc = "2D grid: opacity[i,j] corresponds to log_pressure[i] and log_temperature[j]";
...
int main() {
    ...
    for (const auto& filepath : h5_files) {
        ...
        if (extract_data_from_file(filepath.c_str(), data)) {
            // std::cout << "Extracted data from " << filepath << ": "
            //           << "Temperature: " << data.temperature << ", "
            //           << "Density: " << data.nden << ", "
            //           << "Planck Mean: " << data.planck_mean << ", "
            //           << "Rosseland Mean: " << data.rosseland_mean << std::endl;
    ...
    if (create_2d_opacity_table(opacity_data, output_filename)) {
        ...
        printf("Pressure: %.3e - %.3e erg/cm^3\n", pres.front(), pres.back());

        if (opacity_data.size() == temps.size() * 21) {
            printf("pressure points per temperature: 21\n");
